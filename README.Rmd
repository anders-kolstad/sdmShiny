...
Warning 2: *The response has five or fewer unique values.  Are you sure you want to do regression?*
...
```
I would not make a presense absense map for the Primula with threshold values being so fluctuating. Either I make probability maps for all, or I drop some species out. The AUC is more stable, some some replicated partitioning seems warranted. The trade off here is with processing time for this thing to be able to run live predictions.
...
raster::plot(diffpa)
```
Differences for sure. Close to max difference in Finnmark fo example. 
